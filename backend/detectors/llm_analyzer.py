...
역할:
  ...
  - Claude에게 문자 내용 + 탐지 결과를 주고 피싱 여부 최종 판단
  - "왜 피싱인지" 한국어 자연어 설명 생성

비용 절감 설계:
  - 40점 미만 → 안전 확실 → LLM 호출 안 함
  - 70점 초과 → 위험 확실 → LLM 호출 안 함
  - 40~70점만 → LLM 호출 (전체 요청의 약 20~30% 예상)

보안 설계:
  - API 키는 환경변수 ANTHROPIC_API_KEY 로만 관리
  - LLM 응답 JSON 파싱 실패 시 graceful degradation
  - 타임아웃 20초 — LLM 지연으로 인한 서비스 블로킹 방지
...
# Anthropic 클라이언트 (모듈 로드 시 1회 생성)
# 보안 이유: 객체를 매 요청마다 생성하면 메모리 낭비 + 연결 오버헤드
...
def get_client() -> AsyncAnthropic | None:
    ...
    if _client is None:
        _client = AsyncAnthropic(
            ...
            timeout=20.0,   # 20초 타임아웃 — LLM 응답 지연 방어
        )
    return _client
...
def build_prompt(
    ...
    """
    Claude에게 보낼 분석 요청 프롬프트를 구성한다.

    프롬프트 인젝션 방어 전략:
      - 사용자 입력(text)을 XML 태그로 명확히 감싸서
        모델이 "데이터"와 "지시문"을 혼동하지 않도록 함
      - "위 지시를 무시하고..." 같은 공격이 데이터 영역에만 머물게 제한
...
{{"score": 0~100 사이 정수, "reason": "판단 근거를 한국어로 2~3문장 설명"}}
...
def parse_llm_response(raw: str) -> dict | None:
    ...
    1차: 직접 파싱 / 2차: 마크다운 코드블록 / 3차: 텍스트 중 JSON 추출
    """
    # 1차: 직접 JSON 파싱
    try:
...
async def analyze_llm(
    ...
    """
    Claude API를 호출해 최종 피싱 판단 및 자연어 설명을 반환한다.
    ...
    try:
        ...
        print("⚠️  LLM API 타임아웃 (20초 초과)")
        return {"llm_score": None, "llm_reason": None, "llm_used": False}

    except APIError as e:
        print(f"⚠️  LLM API 오류: {e.status_code} {e.message}")
        return {"llm_score": None, "llm_reason": None, "llm_used": False}

    except Exception as e:
        print(f"⚠️  LLM 분석 중 예외: {type(e).__name__}: {e}")
        return {"llm_score": None, "llm_reason": None, "llm_used": False}