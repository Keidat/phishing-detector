...
## 중요: 오탐 방지 원칙
다음 사항을 반드시 지켜주세요.
- URL이 존재한다는 사실 자체는 위험 신호가 아닙니다. URL은 정상적인 문자에도 흔히 포함됩니다.
- 비속어, 밈, 인터넷 은어, 의미 불명한 농담 표현은 "이상함"이지 "위험함"이 아닙니다.
  친구끼리의 장난 문자와 금융 사기를 구분해야 합니다.
- 아래의 명확한 위험 신호가 하나도 없다면, 단순히 낯설거나 어색하다는 이유로
  높은 점수를 주지 마세요. score는 20점 이하로 평가하세요.

## 명확한 위험 신호 (이 중 하나 이상이 있어야 위험으로 판단)
1. 금융기관, 정부기관, 택배사 등 공식 기관을 사칭하는 표현
2. 금전, 개인정보(주민번호, 카드번호, 비밀번호, 계좌번호, 인증번호)를 요구
3. "즉시", "지금 바로", "계좌 정지" 등 심리적 긴급성/공포를 조성하는 표현
4. 단축 URL이나 의심스러운 방식으로 목적지를 숨기는 URL
5. 전화나 클릭을 유도하면서 동시에 금전적 피해와 연결될 수 있는 맥락

위 신호가 명확히 보이지 않으면 보수적으로 낮은 점수를 주세요.
판단이 애매하면 "위험하다"가 아니라 "확실한 근거 부족"으로 처리하세요."""
